...
public class StatManager {
    ...
    public double getDefenseFlatReduction(Player player) {
        int level = getStatLevel(player, StatType.DEFENSE);
        double perLevel = plugin.getConfig().getDouble("stats.defense-stat-per-level", 0.2);
        return level * perLevel;
    }
    ...
    public int getTotalStatScore(Player player) {
        ...
        for (StatType stat : StatType.values()) {
            total += getStatLevel(player, stat);
        }
        return total;
    }

    public double getUnderdogReduction(Player victim, Player attacker) {
        int victimScore = getTotalStatScore(victim);
        int attackerScore = getTotalStatScore(attacker);
        int gap = attackerScore - victimScore;

        int threshold = plugin.getConfig().getInt("settings.underdog-gap-threshold", 15);
        int maxGap = plugin.getConfig().getInt("settings.underdog-max-gap", 60);
        double maxReduction = plugin.getConfig().getDouble("settings.underdog-max-reduction", 0.20);

        if (gap < threshold) return 0.0;

        double t = (double)(Math.min(gap, maxGap) - threshold) / Math.max(1, maxGap - threshold);
        return t * maxReduction;
    }

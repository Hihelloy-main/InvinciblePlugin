...
public class CombatManager {
    ...
    private final com.hihelloy.invincible.InvinciblePlugin plugin;
    ...
    public CombatManager(com.hihelloy.invincible.InvinciblePlugin plugin, long comboWindowMs, long iframeTicks) {
        this.plugin = plugin;
    ...
    public double getComboMultiplier(int combo) {
        ...
        plugin.getConfig().getConfigurationSection("combat.combo-multipliers");
        if (sec != null) {
            String key = combo <= 8 ? String.valueOf(combo) : "default";
            if (sec.contains(key)) return sec.getDouble(key);
            if (sec.contains("default")) return sec.getDouble("default");
        }
        return switch (combo) {
            case 1 -> 1.00;
            case 2 -> 1.10;
